...
for k, v in {"lang": "ar", "page": "landing", "role": None,
             ...
             "roof": None, "result": None, "b2b_user": None,
             "solar_err": None}.items():
...
def geocode_address(address: str):
    """
    REAL Google Geocoding API: address → ((lat, lon, formatted_address), error).
    Primary : official `googlemaps` SDK.
    Fallback: direct HTTPS via `requests` to the SAME Google endpoint,
              passing key=st.secrets["GOOGLE_API_KEY"] in the params.

    Returns (result, error):
      result = (lat, lon, formatted_address) or None
      error  = None on success;
               "ZERO_RESULTS" if the address genuinely doesn't resolve;
               otherwise a TECHNICAL string with Google's status +
               error_message (e.g. REQUEST_DENIED: API key not authorized).
    NOTE: not cached, so transient errors (quota, network) never stick.
    """
    key = get_google_key()
    if key is None:
        return None, "MISSING_KEY: GOOGLE_API_KEY not found in st.secrets"
        ...
        try:
            ...
            if res:
                ...
                return (loc["lat"], loc["lng"],
                        res[0]["formatted_address"]), None
            return None, "ZERO_RESULTS"
        except Exception as e:             # surface the SDK's own message
    ...
    try:                                   # ---- raw HTTPS path (Google) ----
        r = requests.get(
            ...
            timeout=10)
        js = r.json()
        status = js.get("status", f"HTTP_{r.status_code}")
        if status == "OK":
            top = js["results"][0]
            loc = top["geometry"]["location"]
            return (loc["lat"], loc["lng"], top["formatted_address"]), None
        if status == "ZERO_RESULTS":
            return None, "ZERO_RESULTS"
        # technical failure — include Google's own explanation verbatim
        return None, (f"Geocoding API {status}: "
                      f"{js.get('error_message', 'no details from Google')}"
                      + (f" | SDK: {sdk_err}" if sdk_err else ""))
    except Exception as e:
        return None, (f"Geocoding network error {type(e).__name__}: {e}"
                      + (f" | SDK: {sdk_err}" if sdk_err else ""))


def solar_api_roof_scan(lat: float, lon: float):
    """
    REAL Google Solar API — buildingInsights:findClosest, called with the
    SAME key: params include key=st.secrets["GOOGLE_API_KEY"].
      gross roof : solarPotential.wholeRoofStats.areaMeters2
      net usable : solarPotential.maxArrayAreaMeters2
                   (Google excludes obstacles/edges/shade → obstacles = diff)
      confidence : imageryQuality (HIGH/MEDIUM/LOW → 97/92/88 %)

    Returns (roof_dict, error):
      error = None                on a real Google measurement,
              "NO_COVERAGE"       on 404 (region not in Solar API imagery —
                                  most of MENA today) → labelled estimate,
              technical string    on any other failure (403 API-not-enabled,
                                  400 bad request, 429 quota, network…) —
                                  the UI prints it verbatim; a labelled
                                  estimate keeps the flow alive.
    NOTE: not cached, so transient errors are retried on the next scan.
    """
    def _estimate():
        # deterministic per-location engineering estimate (clearly labelled)
        ...
        return {"gross_m2": float(gross), "obstacles_m2": obst,
                ...
                "confidence": 88 + ((seed >> 16) % 12), "source": "estimate"}
    ...
    try:
        r = requests.get(
            ...
            params={"location.latitude": f"{lat:.6f}",
                    ...
                    "requiredQuality": "LOW",
                    "key": st.secrets["GOOGLE_API_KEY"]},   # ← same single key
            timeout=15)

        if r.status_code == 200:
            js = r.json()
            sp = js.get("solarPotential", {})
            gross = float(sp.get("wholeRoofStats", {}).get("areaMeters2", 0.0))
            ...
            if gross > 0 and net > 0:
                ...
                return ({"gross_m2": round(gross, 1),
                         ...
                         "net_m2": round(net, 1), "confidence": conf,
                         "source": "google"}, None)
            return _estimate(), ("Solar API 200 but empty solarPotential "
                                 "(no roof segments in response)")

        if r.status_code == 404:            # region outside imagery coverage
            return _estimate(), "NO_COVERAGE"

        # any other HTTP status → extract Google's structured error verbatim
        try:
            err = r.json().get("error", {})
            detail = (f"Solar API HTTP {r.status_code} "
                      f"[{err.get('status', '?')}]: "
                      f"{err.get('message', r.text[:200])}")
        except Exception:
            detail = f"Solar API HTTP {r.status_code}: {r.text[:200]}"
        return _estimate(), detail

    except requests.exceptions.Timeout:
        return _estimate(), "Solar API timeout: no response within 15s"
    except Exception as e:
        return _estimate(), f"Solar API network error {type(e).__name__}: {e}"
...
def page_calculator():
    ...
    if st.button(tr("scan_btn"), type="primary", use_container_width=True):
        if not address.strip():
            ...
            with st.status(tr("scanning1"), expanded=False) as s:
                geo, geo_err = geocode_address(address.strip())  # Google Geocoding
                if geo is None:
    ...
    solar_err = st.session_state.get("solar_err")
    if roof.get("source") == "google":
        st.caption(tr("src_google"))
    elif solar_err == "NO_COVERAGE":
        st.caption(tr("src_estimate"))       # expected 404 outside coverage
    else:
        # unexpected TECHNICAL failure → print the exact error for the admin,
        # and label the displayed numbers as an estimate
        if solar_err:
            st.error(f"🔧 {solar_err}")
